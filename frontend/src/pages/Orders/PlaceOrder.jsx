...
const PlaceOrder = () => {
  ...
  const placeOrderHandler = async () => {
    try {
      ...
      navigate(`/order/${res._id}`); // Corrected template literal
    } catch (error) {
      toast.error(error);
    }
  };
              ...
              <table className="w-full border-collapse">
                ...
                <tbody>
                  {cart.cartItems.map((item, index) => (
                    <tr key={index}>
                      <td className="p-2">
                        <Link to={`/product/${item.product}`}> {/* Corrected template literal */}
                          <img
                            src={item.image}
                            alt={item.name}
                      ...
                      <td className="p-2 hidden lg:block mt-3">
                        <Link to={`/product/${item.product}`}>{item.name}</Link> {/* Corrected template literal */}
                      </td>
